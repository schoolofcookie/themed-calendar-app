    ...
    function onClick() {
        ...
        document.body.onclick = function (e) {
            ...
            if ((targetDomObject) && (targetDomObject.classList) && (targetDomObject.classList.contains("dycalendar-today-btn"))) {
                ...
                // new feature 2, code adapted from: https://youtu.be/bNtyTGBtbK4
                var result = confirm('Click OK to add an event for today to the console and Cancel to return to the calendar.');
                if (result == true) {
                    var msg
                    var event = prompt('Type an event for the selected date. Open the console to view your event when you are finished.');
                    if (event == null || event == "") {
                        msg = "No event provided for selected date.";
                    }
                    else {
                        msg = (month + 1) + "/" + date + "/" + year + ": " + event;
                    }
                    console.log(msg);
                }
            }
